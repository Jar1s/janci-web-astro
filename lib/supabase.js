...
/** @returns {{ data: any[]|null, error: object|null }} */
export async function restSelect(table, params = {}) {
  ...
  const res = await fetch(`${supabaseUrl}/rest/v1/${table}?${qs}`, { headers: authHeaders() });
...
export async function storageEnsureBucket(bucket, { public: isPublic = true } = {}) {
  if (!hasSupabase()) return { error: { message: 'Supabase not configured' } };
  const getRes = await fetch(`${supabaseUrl}/storage/v1/bucket/${bucket}`, { headers: authHeaders() });
