...
export function validateBookingSlotsQuery(query = {}) {
  ...
  if (query.startDate !== undefined) {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(startDate)) {
      errors.push('startDate musí mať formát YYYY-MM-DD');
    } else {
      const parsed = new Date(`${startDate}T00:00:00`);
      if (Number.isNaN(parsed.getTime())) {
        errors.push('startDate je neplatný dátum');
      }
    }
  }
...
export function validateBookingCreate(payload = {}) {
  const errors = [];
  const phoneRegex = /^(?:(?:\+421|00421)\s?\d{3}\s?\d{3}\s?\d{3}|0\d{3}\s?\d{3}\s?\d{3})$/;
  const requestIdRegex = /^[a-zA-Z0-9][a-zA-Z0-9._:-]{7,79}$/;
  ...
  if (!payload.phone || typeof payload.phone !== 'string' || payload.phone.trim().length < 6) {
    ...
    const normalizedPhone = String(payload.phone).replace(/[\s-]/g, '').trim();
    if (!phoneRegex.test(normalizedPhone)) {
      errors.push('Telefón musí byť v tvare +421XXXXXXXXX alebo 0XXXXXXXXX');
    }
  }
  ...
  if (payload.recaptchaToken !== undefined && payload.recaptchaToken !== null) {
    const recaptchaToken = String(payload.recaptchaToken).trim();
    if (recaptchaToken.length > 2048) {
      errors.push('reCAPTCHA token je neplatný');
    }
  }

  if (payload.clientRequestId !== undefined && payload.clientRequestId !== null) {
    const requestId = String(payload.clientRequestId).trim();
    if (!requestIdRegex.test(requestId)) {
      errors.push('clientRequestId má neplatný formát');
    }
  }
