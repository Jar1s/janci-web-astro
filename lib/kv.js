...
export async function upsertPartner(payload) {
  ...
  if (error) {
    ...
    if (error.code === 'PGRST204' && /category/i.test(String(error.message))) {
      return {
        ok: false,
        reason: 'schema-migration-required',
        detail:
          'V Supabase chýba stĺpec partners.category. Spusti SQL z súboru supabase/migrations/20260517_partner_category.sql (SQL Editor), potom znova ulož partnera.',
        code: error.code
      };
    }
    return { ok: false, reason: 'db-error', detail: error.message, code: error.code };
...
export async function upsertBookingRequest(payload) {
  if (!hasSupabase() || !hasServiceRole()) {
    return { ok: false, reason: 'missing-service-role' };
  }
  ...
  const row = {
    ...
    slot_start_at: payload.slotStartAt || null,
    ...
    external_booking_id: payload.externalBookingId || null,
    external_response: payload.externalResponse || null,
  ...
  const { error } = await restUpsert(BOOKING_REQUESTS_TABLE, row, 'client_request_id');
  if (error) {
...
export async function getBookingRequestByClientRequestId(clientRequestId) {
  if (!hasSupabase() || !hasServiceRole()) {
    return { bookingRequest: null, reason: 'missing-service-role' };
  }

  const id = String(clientRequestId || '').trim();
  if (!id) return { bookingRequest: null };

  const { data, error } = await restSelectOne(BOOKING_REQUESTS_TABLE, {
    eq: { client_request_id: id }
  });
  if (error) {
    console.error('Error reading booking request by client_request_id:', error);
    return { bookingRequest: null, reason: 'db-error', detail: error.message };
  }
  return { bookingRequest: mapBookingRequest(data) };
}

export async function countRecentBookingRequestsByPhone(phone, hours = 1) {
  if (!hasSupabase() || !hasServiceRole()) {
    return { count: 0, reason: 'missing-service-role' };
  }

  const normalizedPhone = String(phone || '').trim();
  if (!normalizedPhone) return { count: 0 };

  const since = new Date(Date.now() - Math.max(1, Number(hours) || 1) * 60 * 60 * 1000).toISOString();
  const { count, error } = await restCount(BOOKING_REQUESTS_TABLE, {
    eq: { customer_phone: normalizedPhone },
    gte: { created_at: since }
  });

  if (error) {
    console.error('Error counting recent booking requests:', error);
    return { count: 0, reason: 'db-error', detail: error.message };
  }
