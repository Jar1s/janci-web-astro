...
- Reviews: Supabase `reviews` via `/api/reviews` (not live scrape of old kontrolavozidiel.sk); public POST on `/recenzie` (pending approval); admin Recenzie tab; bulk import `npm run reviews:import` from `scripts/data/recenzie-kontrolavozidiel.md`; Google Places only if DB empty.
