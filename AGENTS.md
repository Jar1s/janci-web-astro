...
- `MainLayout.astro` loads site scripts with `defer` so parsing is not blocked.
...
- `AnnouncementBar` only on homepage (`index.astro`), loads `/api/notifications`; ticker ~90–300s; on mobile (≤768px) document flow below navbar (`body.has-announcement-bar`), not fixed under the menu.
